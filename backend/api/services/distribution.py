...
class StudyData:
    ...
    created_at: datetime
    study_type_id: int
    modality: List[str]  # Изменено на List[str] для соответствия модели
    up_value: float
    deadline: datetime
...
class DoctorData:
    ...
    time_start: Optional[datetime] = None  # Добавлено для учета времени начала работы
    time_end: Optional[datetime] = None  # Добавлено для учета времени окончания работы
...
class DistributionService:
    ...
    def get_pending_studies(self) -> List[Study]:
        ...
        Статусы: pending (без назначенного врача)
        """
        return (
            Study.objects.filter(diagnostician__isnull=True, status="pending")
    ...
    def get_available_doctors(self, date: datetime = None) -> List[Doctor]:
        ...
        # Получаем расписания на дату
        schedules = Schedule.objects.filter(
            work_date=date, is_day_off=0
        ).select_related("doctor")

        # Получаем ID врачей из расписаний
        doctor_ids = schedules.values_list("doctor_id", flat=True)

        # Получаем активных врачей
        doctors = Doctor.objects.filter(id__in=doctor_ids, is_active=True)

        return doctors

    def check_modality_compatibility(
        self, study_modalities: List[str], doctor_modalities: List[str]
    ) -> bool:
        """
        Проверить соответствие модальностей врача и исследования.
        ...
        if not study_modalities:
            return True  # Если у исследования не указаны модальности

        # Проверяем пересечение массивов модальностей
        return bool(
            set(s.upper() for s in study_modalities)
            & set(m.upper() for m in doctor_modalities)
        )

    def calculate_completion_time(
        self, doctor: DoctorData, study: StudyData
    ) -> datetime:
        """
    ...
    def find_best_doctor(
        self, study: StudyData, doctors: List[DoctorData]
    ) -> Optional[DoctorData]:
        """
    ...
    def distribute(self) -> Dict:
        ...
        # Подготовка данных врачей с учетом расписания
        doctor_data_list = []
        today = timezone.now().date()

        for doctor in doctors:
            # Получаем расписание врача на сегодня
            schedule = Schedule.objects.filter(
                doctor=doctor, work_date=today, is_day_off=0
            ).first()
        ...
        for study in studies:
            ...
            study_data = StudyData(
                ...
                modality=study_type.modality or []
                if study_type
                else [],  # Изменено на массив
                up_value=float(study_type.up_value)
                if study_type and study_type.up_value
                else 1.0,
            ...
            if best_doctor:
                ...
                completion_time = self.calculate_completion_time(
                    best_doctor, study_data
                )
                tardiness = self.calculate_tardiness(
                    completion_time, study_data.deadline
                )
