...
from .serializers import (
    DoctorSerializer, DoctorWithLoadSerializer,
    StudyTypeSerializer,
    ScheduleSerializer, ScheduleWithDoctorSerializer,
    StudySerializer, StudyWithDetailsSerializer,
    ...
    def with_load(self, request):
        ...
        for doctor in doctors:
            ...
            data.append({
                ...
                'specialty': 'Рентгенолог' if doctor.position_type == 'radiologist' else 'КТ-диагност',
...
class StudyViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def pending(self, request):
        ...
        studies = Study.objects.filter(
            diagnostician_id__isnull=True
        ).select_related('study_type', 'diagnostician')[:100]
...
def dashboard_stats(request):
    ...
    cito_studies = studies_qs.filter(
        priority='cito'
    ).count()
    asap_studies = studies_qs.filter(
        priority='asap'
    ).count()
